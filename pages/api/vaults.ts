...
import type {TVault, TVaultsWithStrat, TStrategy, TStratTree} from './strategies';
...
async function getVaultStrategies({vaultStrategies, stratTree}: {vaultStrategies: TStrategy[], stratTree: TStratTree}) {
	const 	strategies = [];
...
async function getStrategies({network = 1, isCurve = false, isRetired = false, isV1 = false, isAll = false, isStable = false, isDefi = false}): Promise<TVaultsWithStrat[]> {
	...
	if (isRetired) {
		...
		if (isAll) {
			...
			vaults = vaults.filter((e): boolean => (e.apy?.type === 'crv' || e.name?.includes('yvCurve')) ?? false);
		} else {
			vaults = vaults.filter((e): boolean => e.apy?.type !== 'crv');
			vaults = vaults.filter((e): boolean => !e.name?.includes('yvCurve'));
		}
		vaults = vaults.filter((e): boolean => !e.migration || !e.migration.available);
		vaults = vaults.sort((e): 1 | -1 => e.symbol === 'yvBOOST' ? -1 : 1);
	}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
	...
	const	result = await getStrategies({network: Number(network), isCurve: !!isCurve, isRetired: !!isRetired, isV1: !!isV1, isAll: !!isAll, isStable: !!isStable, isDefi: !!isDefi});
